...
(the rules index's tree, which is where `parent_slug` came from), `statute`, or `das`. It is
...
other entry is curation the refresh carries across untouched. A relation with no source is
