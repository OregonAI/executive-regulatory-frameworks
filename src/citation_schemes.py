...
# (20 USC 1232g, FERPA) against 66 distinct sections, measured 2026-09-12 over this
# corpus's agencies/, executive-orders/, constitution/ and external-references/ (the
# scope the originating issue used) — NOT the 784 USC targets (52 with authority claims)
# `_meta/catalog/external-citations.yml` records at full-corpus scope (all 81,921
# documents, generated by src/scan_external_citations.py). Both figures are correct at
# their own scope; quoting one against the other's scope is the discrepancy, not either
# number. So most of the 66 citations still resolve to nothing — that is
# `federal-reference`'s coverage, measured and expected, not a defect here. What this
# registration fixes is that the ones it DOES
...
# match first. federal-reference's ADR-0004 rule survives that repo's own ADR-0006
# supersession: a U.S.C. section is never mapped onto the public law that enacted it.
...
def _proof_federal_schemes_survive_registration(ck, fw):
    ...
    live in the `federal-reference` sibling corpus, reached over the network via the
    `siblings:` entry `_meta/corpus.yml` already declares for it (:153-156, with a live
    `index_url`). This gate refuses network calls and should not depend on one to stay
    green, so it stops at `_match_schemes` — the same function `resolve_citation` calls
    before it ever reaches the network — which proves the served regex, not this
    module's own copy of it."""
    cases = [
...
def _proof_usc_scheme_derives_the_right_section_id(ck, fw):
    ...
    candidates live in `federal-reference`, reached via the `siblings:` entry
    `_meta/corpus.yml` already declares for it (:153-156, with a live `index_url`). This
    gate refuses network calls and should not depend on one to stay green.
    ...
    warns about, not a hypothetical:
      ...
      * a citation naming BOTH a U.S.C. section and a public law resolves to the U.S.C.
        ...
        surface a `pl-` id here. federal-reference's ADR-0004 rule survives that repo's
        own ADR-0006 supersession: a codified section is never mapped onto the enacted
        public law that created it.

    Code review caught this last group PASSING WITH `federal-usc` REMOVED from the
    registration below: `federal-public-law`'s pattern alone already matches a string
    naming both, and it calls the identical `candidates()` function, which resolves to
    the U.S.C. id regardless of which scheme invoked it. So the two `cands ==` /
    `not in cands` checks prove `federal_ids.candidates()`'s own USC-before-PUBLAW
    ordering — true on origin/main, unchanged by this diff — not the new registration.
    The `"federal-usc" in names` check below is the one that actually depends on it:
    remove `federal-usc` from the registration and `_match_schemes` returns
    `"federal-public-law"` alone for this citation, so only that check fails."""
    ...
    ck("a lowercase citation with a hyphen suffix derives the section, not the "
       "different, real section '1320d' would name",
       cands == ["42-usc-1320d-2"])

    _, _, cands, _ = fw._match_schemes("29 USC § 3101")
    ck("the '§' form derives the same shape of id", cands == ["29-usc-3101"])

    names, _, cands, note = fw._match_schemes("42 U.S.C. 1320d-2 (Pub. L. 104-191)")
    ck("federal-usc is one of the schemes that matched this compound citation, not "
       "just federal-public-law -- unlike the two checks below, this one actually "
       "fails if federal-usc's registration is removed",
       "federal-usc" in names.split("+"))
