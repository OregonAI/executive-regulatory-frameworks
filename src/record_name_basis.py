...
**Name-basis backlog**: some rows are carrying a rules-index title under a field that says
otherwise (#279, re-measured for #281). `catalog_agencies.py --check` prints the current
count on every run -- the `name:` line's `unverified-oar-title` tally -- so this docstring
cites that command instead of holding its own copy of the number for something else to let
go stale (#340; #299 and #306 are the history of the copy this replaces).
...
## the figure below is gated separately, by catalog_agencies.py's own
## `chapter-page-count-current` rule (#279), which requires this exact adjacency
## ("re-fetches ... N chapter pages" on one line) -- left outside any `**Term**:` block
## on purpose, so `stated_census.py` does not ALSO try to gate a figure that already has
## a dedicated, working check reading this same sentence. Without this marker the figure
## would fall inside the glossary block above and demand a tag for a fact something else
## already enforces (#340).
...
**Refresh footprint vs. registry size**: not every row carries an `oar_chapter` --
`catalog_agencies.py --check` prints the chaptered-vs-total split on every run (the
`chapter pages:` line), the same reason and the same check `expand_oar_name.py`'s docstring
cites for its own copy of this sentence (#279) — and adding a key to committed data is not
a reason to re-open what every other field says. `catalog_agencies.py --refresh` writes
this key from now on, because `scraped_entry()` does.
...
**Name-basis backlog (reviewed authorities)**: some rows carry a reviewed enabling
authority, including all of the manual ones -- `catalog_agencies.py --check` prints the
current count on every run (the `enabling authority:` line's `recorded` tally). Carrying an
authority is not the same as having read a name off it: the authority answers "what created
this body", and the name answers "what does that authority call it". Those same rows can now
be reviewed for the second question; none of them has been by this script, and recording
them as anything but unverified would credit a review nobody did.
