...
# `name` and `oar_name` hold identical bytes on 187 of the 190 committed rows (#279,
# re-measured for #281), differing only on the three whose established statutory name is
# not the rules index's title (#168), so a search proof that reads the committed registry
# passes whichever field the matcher reads on all but three of them. Both names
