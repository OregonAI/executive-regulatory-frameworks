...
ONLY EVIDENCE IS DERIVED FROM, NEVER A FAILED SEARCH, AND THAT IS THE WHOLE OF CONTEXT.md'S
OVERRIDING RULE. A candidate is evidence that a body is separately constituted, so a body
with one becomes `administered_by`; a REVIEWED absence is evidence too, of the opposite kind
(#222, below), so a body carrying `none: <reason>` becomes `part_of`; every body with
neither stays `undetermined` (the live split of all three, and of the basis each decided
kind rests on, is `relation_census()`'s count, printed by `catalog_agencies.py --check` on
every run, rather than a figure fixed here). NOTHING here reads the mere ABSENCE of a
candidate as evidence of `part_of`: the matcher finding nothing is a
...
A REVIEWED ABSENCE DERIVES `part_of`, AND THIS IS THE SECOND DERIVATION #173 DEFERRED (#222).
`none: <reason>` on a row means a human looked and found no separate authority, which is ADR
0004's own description of a *part of* unit: nothing separately constitutes the body, so
there is nothing separate to enable. That is EVIDENCE, at the strength of a reviewed
authority — a human read the body's law and recorded what they found — and it is not the
same fact as the ABSENCE OF A CANDIDATE this module refuses to derive from above. The two
are opposite ends of "somebody looked": a `no_candidate` row is a statement about the
matcher, and a `none: <reason>` row is a statement about the body, made by a human who read
its law and found no separate authority to record. Confusing them would be exactly the
failure this module's other half exists to prevent, one field over — so a reviewed absence
is checked BEFORE the sheet is ever consulted (`decide()` reads `enabling_authority` first),
and the basis it derives, `reviewed-absence`, cannot be produced any other way. The relation
carries no `authority`: ADR 0004's *part of* cites none, because there is nothing separate
to cite, and `catalog_agencies.relation_fault()` refuses one on a `part_of` row for exactly
that reason.

A reviewed absence retires the proposal too, on any row that also carries one: a candidate a
human has ruled on is not a proposal any more, so `decide()` never falls through to the
sheet for such a row regardless of which kind it derives.
...
from catalog_agencies import (ADMINISTERED_BY, DECISION_KEYS, PART_OF, PROPOSED_AUTHORITY,
                              RELATION_KEY, REVIEWED_ABSENCE, REVIEWED_AUTHORITY,
                              UNDETERMINED, classify_authority, relation_census,
                              relation_entries)
...
# ACCEPTS. They hold the same three values today, and naming them separately is for the day
...
DERIVED_BASES = (PROPOSED_AUTHORITY, REVIEWED_AUTHORITY, REVIEWED_ABSENCE)
...
def decide(org, proposed) -> Decision | None:
    ...
    A REVIEWED ABSENCE DECIDES THE OTHER KIND (#222). `enabling_authority` holding
    `none: <reason>` is a human's own statement that nothing separately constitutes this
    body — ADR 0004's *part of* — and it is checked here, before the sheet is ever
    consulted, so a proposal a reviewer has already rejected can never be used instead.
    """
    ...
    if "enabling_authority" in org:
        ...
        if form is None:
            ...
            # that gate is about to refuse. Nothing is decided from it here — including the
            # malformed shape of a reviewed absence: `none` with no reason, or a reason too
            # short to be one, is not evidence of anything.
            return None
        if form == "reviewed-none":
            # A HUMAN LOOKED AND FOUND NOTHING SEPARATE. `authority` is None: ADR 0004's
            # *part of* cites nothing, because there is nothing separate to cite, and
            # `catalog_agencies.relation_fault()` refuses a `part_of` relation that carries
            # one.
            return Decision(PART_OF, REVIEWED_ABSENCE, None)
...
def apply_decisions(cat, proposed) -> int:
    ...
    A `part_of` DECISION CARRIES NO `authority` KEY AT ALL, never `authority: null` — the
    two read alike to a plain equality check but not to `relation_fault()`, which refuses
    ANY `authority` key on a `part_of` relation regardless of its value (ADR 0004: nothing
    separate to cite). So `decision.authority is None` is read as "pop the key", not "set it
    to None", and the comparison against the entry's current state includes whether the key
    is even PRESENT — `entry.get("authority")` alone cannot tell a popped key apart from one
    holding a literal `null`, and this is the one place that distinction is load-bearing.
    """
    ...
    for org in cat.get("organizations") or []:
        ...
        for entry in relation_entries(org):
            ...
            if decision is None:
                ...
                continue
            has_authority = decision.authority is not None
            was = (entry.get("kind"), entry.get("basis"), entry.get("authority"),
                   "authority" in entry)
            want = (decision.kind, decision.basis, decision.authority, has_authority)
            if was != want:
                entry["kind"] = decision.kind
                entry["basis"] = decision.basis
                if has_authority:
                    entry["authority"] = decision.authority
                else:
                    entry.pop("authority", None)
                touched = True
...
def audit(orgs, proposed, verdicts) -> list:
    ...
    # A REVIEWED ABSENCE THAT LATER GAINS A CANDIDATE IS A CONFLICT, REPORTED RATHER THAN
    # SILENTLY RE-DERIVED (#222's fourth criterion). `decide()` above checks
    # `enabling_authority` before it ever consults the sheet, on purpose: a human's reviewed
    # absence must not be overridden by an unread proposal just because a later matcher pass
    # found one. But that means such a proposal is invisible to everything above unless it is
    # reported here -- the no-candidate list moving 118 -> 63 in one session (#173, #159) is
    # the measured precedent for a later pass finding what an earlier one missed, and the row
    # must surface that rather than let `part_of` stand unexamined while a live candidate sits
    # beside it in the sheet.
    for slug, org in sorted(by_slug.items()):
        if slug not in decides_for or "enabling_authority" not in org:
            continue
        form, _detail = classify_authority(org["enabling_authority"])
        if form != "reviewed-none":
            continue
        citation = proposed.get(slug)
        if citation and recordable(citation):
            problems.append(Problem(
                "reviewed-absence-conflicts-with-candidate", slug,
                f"enabling_authority records a reviewed absence "
                f"({org['enabling_authority']!r}) but the review sheet still proposes "
                f"{citation!r} for this body -- a later matcher pass found a candidate for a "
                "body already reviewed as having none. That is a conflict for a human to "
                "resolve, not something this derivation may pick a side of: re-review the "
                "proposal against the reviewed absence and update whichever one is wrong"))
...
def check() -> int:
    ...
    kinds = collections.Counter(d.kind for d in decisions.values())
    # THE THREE POPULATIONS, NAMED APART AND ALL COUNTED, on every run — the zeroes
    # included (#222: running against the committed registry today derives zero `part_of`
    # rows, and printing that as a bare absence would read as "no bodies are parts" rather
    # than "nobody has recorded a reviewed absence yet"). The number that would be easiest
    # to print is a single administered-by tally, and it is the one this module may not
    # print alone: some of that population rest on candidates nobody has read, and the count
    # of bodies left `undetermined` is not a backlog to be quiet about — it is the answer for
    # every body no evidence speaks to.
    ...
    print(f"  {ADMINISTERED_BY}: {kinds[ADMINISTERED_BY]} — "
          f"{bases[PROPOSED_AUTHORITY]} from a PROPOSED candidate nobody has read, "
          f"{bases[REVIEWED_AUTHORITY]} from a REVIEWED enabling authority")
    print(f"  {PART_OF}: {kinds[PART_OF]} — {bases[REVIEWED_ABSENCE]} from a REVIEWED "
          "ABSENCE (a human looked and found no separate authority to record); never from "
          "the mere absence of a candidate")
    return 0
...
def _registry():
    ...
    orgs = [
        ...
        # A REVIEWED ABSENCE, NOTHING LEFT PROPOSED: a human ruled on this one and the sheet
        # holds no candidate for it, which is the ordinary case -- the matcher never had
        # anything to say and the review confirms it. The `part_of` this supports is a
        # derivation this ticket did not take.
...
def _case_malformed_reviewed_absence_after_apply(f):
    """A registry that already carries `part_of` from a reviewed absence, whose row is then
    edited to the marker alone (`none`, no `: <reason>`). This is the acceptance criterion's
    own case, watched failing at the GATE rather than only at `decide()`: the fixture's
    `imaginary-affairs-records-unit` already holds `kind: part_of, basis: reviewed-absence`
    from `_fixture()`'s own --apply, so the row is stale evidence of a decision this module
    would no longer make — and --check must refuse it rather than accept a `part_of` resting
    on a value its own contract is about to reject."""
    _org(f, "imaginary-affairs-records-unit")["enabling_authority"] = "none"


def _case_reviewed_absence_gains_a_later_candidate(f):
    """#222's fourth criterion. `imaginary-affairs-records-unit` already carries a reviewed
    absence; this adds a sheet candidate for it, the shape a LATER matcher pass produces when
    it finds something an earlier one missed (the no-candidate list moving 118 -> 63 in one
    session is the measured precedent). `decide()` must keep deriving `part_of` from the
    human's reviewed absence rather than the unread proposal -- but the conflict must not go
    unreported while `part_of` stands unexamined beside a live candidate."""
    f["sheet"]["candidates"].append(
...
_CASES = [
    ...
    ("malformed-reviewed-absence-after-apply", _case_malformed_reviewed_absence_after_apply,
     "kind-agrees"),
    ...
    ("decision-reached-some-other-way", _case_decision_reached_some_other_way,
     "decision-not-ours"),
    ("reviewed-absence-gains-a-later-candidate",
     _case_reviewed_absence_gains_a_later_candidate,
     "reviewed-absence-conflicts-with-candidate"),
]


def _proof_evidence_derives_and_a_failed_search_does_not() -> int:
    """The derivation decides `administered_by` from evidence of the body, `part_of` from
    evidence of the body, and NOTHING from a failed search. The fixture holds one child no
    candidate speaks for at all — the matcher missed it, nobody has reviewed it — and it
    alone must come out `undetermined`: an absent candidate is a statement about the matcher,
    ...
    expected = {
        ...
        "imaginary-affairs-records-unit": (PART_OF, REVIEWED_ABSENCE, None),
    ...
    for slug, want in expected.items():
        ...
        if got != want:
            print(f"FAIL evidence-derives-and-a-failed-search-does-not: {slug} -> {got!r}, "
                  f"expected {want!r}", file=sys.stderr)
            bad += 1
        if want[0] == PART_OF and "authority" in entry:
            print(f"FAIL part-of-carries-no-authority-key: {slug} -> {entry!r}",
                  file=sys.stderr)
            bad += 1
    if any(e.get("kind") == PART_OF
           for e in _org(f, "imaginary-affairs-inspection-division")[RELATION_KEY]):
        print("FAIL no-part-of-from-a-mere-absent-candidate: a kind was derived from the "
              "matcher's silence rather than from a human's reviewed absence", file=sys.stderr)
        bad += 1
    return bad


def _proof_malformed_reviewed_absence_derives_nothing() -> int:
    """The marker without a reason — `none` alone, with no `: <reason>` — is not a reviewed
    absence. `classify_authority()` refuses it (that is `enabling-authority-form`'s failure
    to report), and deriving `part_of` from a value the registry's own contract is about to
    reject would be a decision resting on nothing, which is the exact failure a `basis`
    exists to make visible rather than the exact failure this module produces one more of."""
    f = _fixture()
    _org(f, "imaginary-affairs-records-unit")["enabling_authority"] = "none"
    decision = decide(_org(f, "imaginary-affairs-records-unit"), read_sheet(f["sheet"]).proposed)
    if decision is not None:
        print(f"FAIL malformed-reviewed-absence-derives-nothing: decide() returned "
              f"{decision!r} for an unreadable value", file=sys.stderr)
        return 1
    return 0
...
def _proof_apply_writes_the_same_bytes_twice() -> int:
    ...
    # The three rows the derivation decides are the three it may write — two
    # `administered_by` and, since #222, one `part_of` from the reviewed absence. A run
    # that touched the remaining row wrote `undetermined` over `undetermined` and would
    # churn the file forever.
    if first != 3 or again != 0 or once != twice:
        print(f"FAIL apply-writes-the-same-bytes-twice: wrote {first} of 3 row(s), rewrote "
...
def selftest() -> int:
    bad = (_proof_evidence_derives_and_a_failed_search_does_not()
           + _proof_malformed_reviewed_absence_derives_nothing()
    ...
    for name, mutate, rule in _CASES:
        ...
        if not any(p.rule == rule for p in problems):
            print(f"FAIL {name}: expected a [{rule}] problem, got {problems}",
                  file=sys.stderr)
            bad += 1
    print(f"{len(_CASES)} violation(s) demonstrated failing, 5 derivation proof(s) held"
